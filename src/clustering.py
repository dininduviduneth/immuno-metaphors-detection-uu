...
def main():
    ...
    for idx, item in enumerate(pcacontext):
        print(f"Distance to centroid 1 from {context_words[idx]} was {np.linalg.norm(item - centroids[0])}")
        print(f"Distacne to centroid 2 from {context_words[idx]} was {np.linalg.norm(item - centroids[1])}")
